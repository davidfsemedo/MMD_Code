...
class PrepareData:
        ...
        if create_vocab or not os.path.exists(dialogue_pkl_file):
            if self.task_type == "text":
                self.binarize_text_corpora(self.dialogue_context_text_task_text_file,
                                           self.dialogue_context_image_task_text_file,
                                           self.dialogue_target_text_task_text_file, self.task_type, dialogue_pkl_file)
            if self.task_type == "image":
                self.binarize_image_corpora(self.dialogue_context_text_task_image_file,
                                            self.dialogue_context_image_task_image_file,
                                            self.dialogue_target_image_task_image_pos_file,
                                            self.dialogue_target_image_task_image_negs_file, dialogue_pkl_file)
                        ...
                        if dialogue_instance_multimodal[-1]['false images'] is not None:
                            ...
                            for img in dialogue_instance_multimodal[-1]['false images']:
                                ...
                                if dialogue_target_images_pos and img in dialogue_target_images_pos and len(
                                    ...
                                    dialogue_target_images_negs.append(img)
                        if (dialogue_target_images_pos is None or len(dialogue_target_images_pos) == 0) and (
                                dialogue_target_images_negs is None or dialogue_target_images_negs is None):
                            continue
                        if dialogue_target_images_pos is not None:
                            if (dialogue_target_images_negs is None or len(
                                    dialogue_target_images_negs) < self.max_negs):
    ...
    def binarize_image_corpora(self, dialogue_file_text, dialogue_file_image, dialogue_target_file_pos,
                               dialogue_target_file_negs, dialogue_pkl_file):
        binarized_corpus = []
        ...
        with open(dialogue_file_text) as textlines, open(dialogue_file_image) as imagelines, open(
                dialogue_target_file_pos) as targetposlines, open(dialogue_target_file_negs) as targetnegslines:
            ...
            for text_context, image_context, target_pos, target_negs in zip(textlines, imagelines, targetposlines,
                                                                            targetnegslines):
                ...
                binarized_corpus.append(
                    [binarized_text_context, binarized_image_context, [binarized_target_pos], binarized_target_negs,
                     mask_target_negs])
