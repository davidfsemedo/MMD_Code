...
def get_test_loss(sess, model, batch_dict, param, loss, cosine_sim_pos, cosine_sim_negs):
    test_batch_text, test_batch_image, batch_image_target_pos, batch_image_target_negs, batch_mask_negs = get_batch_data(
        param['max_len'], param['max_images'], param['image_rep_size'], param['max_utter'], param['max_negs'],
        param['batch_size'], batch_dict)
    feed_dict = feeding_dict(model, test_batch_text, test_batch_image, batch_image_target_pos, batch_image_target_negs,
                             batch_mask_negs)
...
def perform_test(sess, model, param, loss, cosine_sim_pos, cosine_sim_negs):
    ...
    for i in range(n_batches):
        ...
        batch_mm_loss, batch_cosine_sim_pos, batch_cosine_sim_negs, batch_mask_negs = get_test_loss(sess, model,
                                                                                                    batch_dict, param,
                                                                                                    loss,
                                                                                                    cosine_sim_pos,
                                                                                                    cosine_sim_negs)
        batch_test_loss, count_batch, recall_1, total_1, recall_2, total_2, recall_3, total_3, recall_4, total_4 = get_loss(
            param, batch_cosine_sim_pos, batch_cosine_sim_negs, batch_mask_negs)
        ...
        try:
            if total_1 > 0 and total_2 > 0 and total_3 > 0 and total_4 > 0:
                print('recall@1=%.6f recall@2=%.6f recall@3=%.6f recall@4=%.6f' % (
                recall_1 / total_1, recall_2 / total_2, recall_3 / total_3, recall_4 / total_4))
        except:
            continue
    # break
    ...
    print(
        'average test loss is = %.6f/%.6f = %.6f   average MM loss =%.6f (cosine-dist-pos=%.6f cosine-dist-neg=%.6f) ' % (
        sum_test_loss, count_test, avg_test_loss, avg_mm_loss, avg_cosine_sim_pos, avg_cosine_sim_neg))
...
def get_loss(param, cosine_sim_pos, cosine_sim_negs, mask_negs):
    ...
    for i in range(param['batch_size']):
        print
        cosine_sim_pos[i], cosine_sim_negs[i]
    ...
    return float(count_incorrect), float(count), float(recall_1), float(total_1), float(recall_2), float(
        total_2), float(recall_3), float(total_3), float(recall_4), float(total_4)
...
def run_test(param):
    ...
    print
    'valid target sentence list loaded'
    print
    ...
    with tf.Graph().as_default():
        ...
        if old_model_file is not None:
            print
            "best model exists.. restoring from that point"
